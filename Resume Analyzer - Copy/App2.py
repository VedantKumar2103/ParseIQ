...
openai.api_key = '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'  # Replace with your actual API key

# ML Model for Resume Classification~
def train_resume_classifier():
    # Sample training data (in a real app, you'd have a proper dataset)
    resumes = [
...
def highlight_sections(resume_text):
    """Analyze and highlight missing/weak sections in the resume"""
    sections = {
        'Objective': False,
        'Skills': False,
        'Experience': False,
    ...
    # Simple check for section headers (could be enhanced with NLP)
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            # Show PDF with tabs
            tab1, tab2 = st.tabs(["Resume Preview", "Resume Analysis"])
            ...
            with tab2:
                ...
                if resume_data:
                    ...
                    # ML Classification
                    domain_prediction = classify_resume(resume_text)
                    st.success(f"🔍 Our ML model predicts this resume is for: {domain_prediction}")
                    ...
                    highlight_sections(resume_text)
                    ...
                    job_desc = st.text_area("Paste the job description here (optional):", height=100)
                    ...
                    # Resume scoring and other existing features...
                    ts = time.time()
                    ...
                    if 'Objective' in resume_text:
                        ...
                        st.markdown('''<h4 style='text-align: left; color: #fabc10;'>[-] Add a career objective</h4>''', unsafe_allow_html=True)

                    if 'Declaration' in resume_text:
                        resume_score += 20
                        st.markdown('''<h4 style='text-align: left; color: #1ed760;'>[+] Awesome! You have added Declaration✍</h4>''', unsafe_allow_html=True)
                    else:
                        st.markdown('''<h4 style='text-align: left; color: #fabc10;'>[-] Add a declaration</h4>''', unsafe_allow_html=True)

                    if 'Hobbies' in resume_text or 'Interests' in resume_text:
                        resume_score += 20
                        st.markdown('''<h4 style='text-align: left; color: #1ed760;'>[+] Awesome! You have added Hobbies⚽</h4>''', unsafe_allow_html=True)
                    else:
                        st.markdown('''<h4 style='text-align: left; color: #fabc10;'>[-] Add hobbies/interests</h4>''', unsafe_allow_html=True)
