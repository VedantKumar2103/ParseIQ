...
def load_css(file_path):
    """Load and inject CSS styles from a file."""
    with open(file_path) as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

# Load the CSS file
load_css(r"C:\Users\vedan\Projects\ParseIQ\assests\style.css")
...
def run():
    ...
    # Use st.expander to hide the "Choose User" section
    ...
    if choice == "Normal User":
        st.subheader("Normal User Section")
        # Add your Normal User-specific content here
    elif choice == "Admin":
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                # ... (all your existing imports and code remains the same until the video sections)
                ...
                resume_vid = random.choice(resume_videos)  # Select a random video from the resume_videos list
                resume_vid_title = fetch_yt_video(resume_vid)
                st.subheader("✅ **" + resume_vid_title + "**")
                st.video(resume_vid)

                connection.commit()

                # New section for ATS-friendly resume template download
                st.header("**ATS-Friendly Resume Template**")
                st.markdown('''<h4 style='text-align: left; color: #1ed760;'>
                    Would you like to download an ATS-friendly resume template to improve your resume?</h4>''', 
                    unsafe_allow_html=True)
                
                if st.button("Download ATS-Friendly Resume Template"):
                    resume_path = r"C:\Users\vedan\Projects\ParseIQ\ATS Friendly Resume\ATS Friendly CV Template.pdf"
                    try:
